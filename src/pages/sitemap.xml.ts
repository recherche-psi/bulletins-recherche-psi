...
export async function GET({ site }: { site: URL }) {
  ...
  const paths = ['vulgarisation/', ...articles.map(post => `vulgarisation/${post.id}/`), '', 'bulletins/', 'methode/', 'a-propos/', 'en/', 'en/bulletins/', 'en/method/', 'en/about/', ...translations.map(post => `en/bulletins/${post.id}/`), ...posts.map((post) => `bulletins/${post.id}/`)];
